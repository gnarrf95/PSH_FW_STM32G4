...
static inline void DMARX_Reset(SerialHandler_Instance_t *pThis)
{
	...
	DMARX_INSTANCE->IFCR |= 0xF << DMARX_INDEX;
}
...
ErrorStatus SerialHandler_SendData(SerialHandler_Instance_t *pThis, uint8_t *pData, uint32_t dataSize)
{
	...
	DMATX_INSTANCE->IFCR |= 0xF << DMATX_INDEX;
...
{
	if(DMARX_INSTANCE->ISR & (0x2 << DMARX_INDEX))
	{
		USART_INSTANCE->CR3 &= ~USART_CR3_DMAR;
	...
	if(DMARX_INSTANCE->ISR & (0x8 << DMARX_INDEX))
	{
		USART_INSTANCE->CR3 &= ~USART_CR3_DMAR;
...
{
	// TC Flag
	if(DMATX_INSTANCE->ISR & (0x2 << DMATX_INDEX))
	{
		USART_INSTANCE->CR3 &= ~USART_CR3_DMAT;
		DMATX_CHANNEL->CCR &= ~DMA_CCR_EN;
		...
		DMATX_INSTANCE->IFCR |= 0x2 << DMATX_INDEX;
	}

	// Error Flag
	if(DMATX_INSTANCE->ISR & (0x8 << DMATX_INDEX))
	{
		USART_INSTANCE->CR3 &= ~USART_CR3_DMAT;
		DMATX_CHANNEL->CCR &= ~DMA_CCR_EN;

		DMATX_INSTANCE->IFCR |= 0x8 << DMATX_INDEX;
	}

	DMATX_INSTANCE->IFCR |= 0x1 << DMATX_INDEX;
}
